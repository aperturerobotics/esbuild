...
type parser struct {
	...
	//   })(ns || (ns = {}));
	//
	// Relevant issue: https://github.com/aperturerobotics/esbuild/issues/1158
				...
				case "declare":
					if !p.lexer.HasNewlineBefore && opts.isClass && p.options.ts.Parse && opts.tsDeclareRange.Len == 0 {
						...
						if prop, ok := p.parseProperty(startLoc, kind, opts, nil); ok &&
							...
							// References:
							//
							//   https://github.com/aperturerobotics/esbuild/issues/1675
				...
				case "abstract":
					if !p.lexer.HasNewlineBefore && opts.isClass && p.options.ts.Parse && !opts.isTSAbstract {
						...
						if prop, ok := p.parseProperty(startLoc, kind, opts, nil); ok &&
							...
							// References:
							//
							//   https://github.com/aperturerobotics/esbuild/issues/3684
							//
							prop.Kind = js_ast.PropertyDeclareOrAbstract
							return prop, true
						}
		...
		default:
			...
			case js_ast.UnOpVoid:
				...
				if p.options.minifySyntax {
					...
					//
					// More info: https://github.com/aperturerobotics/esbuild/issues/4041
