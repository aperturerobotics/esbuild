...
func (originalParser *parser) isTypeScriptArrowReturnTypeAfterQuestionAndBeforeColon(await awaitOrYield) bool {
	...
	p := newParser(logger.NewDeferLog(logger.DeferLogNoVerboseOrDebug, logger.LevelError, nil), originalParser.source, originalParser.lexer, &originalParser.options)
...
func (p *parser) parseTypeScriptNamespaceStmt(loc logger.Loc, opts parseStmtOpts) js_ast.Stmt {
	...
	// in that case. See https://github.com/aperturerobotics/esbuild/issues/1158.
